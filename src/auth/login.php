<?php
    ...
    function login(): void{
        ...
        if($result->num_rows != 1){
            echo '<div aria-live="assertive" class="notification notification--failure no-animate text-small">User not found</div>';
            throw new Exception();
        }
        ...
        if(!$user->verifyPassword($inputPass)){
            echo '<div aria-live="assertive" class="notification notification--failure no-animate text-small">Wrong email or password</div>';
            throw new Exception();
        }
